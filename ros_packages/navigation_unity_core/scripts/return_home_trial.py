...
class DefaultWorldLoader(object):
    ...
    def mainloop(self):
        ...
        # TODO - extract the explored areas from a given dataset automatically
        # TODO - add topic on which the robot can publish the answer like (waypoint reached) and check for that here
        zones = ["corner1", "step", "corner2", "shortbeach"]
        returning_times = [60, 50, 70, 50]
        home_zone = "center"
        ...
        for i in range(len(zones)):
            ...
            print("-SPAWNING, AREA: " + zones[i] + "SPAWNPOINT: " + str(yamldata['spawn_point_number']) + " TIME: " + str(returning_times[i]) + "s")
            reset_world = rospy.ServiceProxy('reset_world', ResetWorld)
            reset_world(updated_yaml_string)

            # wait
            rospy.sleep(returning_times[i])
